...
namespace SR.ModRimWorldTouchCat
{
    ...
    public class JobGiverTouchRabbit : ThinkNode_JobGiver
    {
        ...
        /// </summary>
        /// <returns></returns>
        private static Pawn FindRabbit(Pawn pawn)
        {
            //尝试在附近寻找兔子
            ...
            foreach (var anyPawn in currentMap.mapPawns.AllPawnsSpawned)
            {
                ...
                //当前pawn种族不是兔子
                if (!anyPawn.kindDef.race.defName.Equals(ModDef.RaceDefNameRabbit) &&
                    !anyPawn.kindDef.race.defName.Equals(ModDef.RaceDefNameSnowRabbit))
                {
                    continue;
                }
